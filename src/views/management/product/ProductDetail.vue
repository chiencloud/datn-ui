        ...
        <div class="product-detail-attribute-area">
          ...
          >
            <div class="product-detail-attribute-color">
              <div class="attribute-title mr-5">Phân loại hàng 1</div>
              <v-text-field
                label="Tên nhóm phân loại hàng 1"
                :disabled="isViewMode"
                v-model="currentData.level1name"
                :rules="[rules.productCodeRule]"
              ></v-text-field>
              <div class="attribute-color-content">
                ...
                <v-btn
                  ...
                  :disabled="isViewMode || !currentData.level1name"
                >Thêm dòng</v-btn>
              </div>
            </div>
            <div class="product-detail-attribute-size">
              <div class="attribute-title mr-4">Phân loại hàng 2</div>
              <v-text-field
                label="Tên nhóm phân loại hàng 2"
                :disabled="isViewMode"
                v-model="currentData.level2name"
                :rules="[rules.productCodeRule]"
              ></v-text-field>
              <div v-for="(item, index) in listSizeSelected1" :key="index" class="attribute-size-content">
                <v-text-field
                  v-model="item.value"
                  :disabled="isViewMode"
                  width="300px"
                  @change="changeItemLevel2"
                  clearable
                ></v-text-field>
                <!-- <v-combobox
              ...
              <v-btn
                @click="insertLevel2Attribute()"
                :disabled="isViewMode || !currentData.level2name"
              >Thêm dòng</v-btn>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeItemLevel2(){
      this.listSizeSelected = this.listSizeSelected1.map( x => x.value);
    },
     ...
     * Thêm 1 màu sắc
     */
     insertLevel2Attribute() {
      this.listSizeSelected1.push({
        color: "",
      });
    },
    /**
    ...
    getAllBranch() {
      const me = this;
      BranchService.getAllDataByShop(this.userInfo?.idshop)
        .then((result) => {
          if (result && result.data) {
