...
export default {
  ...
  created() {
    this.userInfor = JSON.parse(localStorage.getItem("user"))?.userInfo;
    this.getDefaultData();
    this.getAllEmployee();
    this.getAllProvince();
  },
  ...
  methods: {
    ...
    getDefaultData() {
      ...
      BranchService.getPagingData({
        ...
        StoreProcedure: "Proc_GetListBranchByShopID",
        idshop: this.userInfor?.idshop,
        ListOrderBy: [],
      }).then((result) => {
        if (result && result.data) {
    ...
    insertBranch() {
      const me = this;
      this.addBranch.idshop = this.userInfor.idshop;
